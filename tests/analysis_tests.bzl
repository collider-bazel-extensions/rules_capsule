"""Analysis-time tests for the rules. Verify rules instantiate cleanly and
expose the expected DefaultInfo without needing kubectl / a real cluster."""
...
def capsule_install_test_suite(name):
    capsule_install(
        name = name + "_subject",
        tags = ["manual"],
    )
...
def capsule_health_check_test_suite(name):
